...
- **Removed Insecure Authentication Fallback**: Removed the insecure `DEMO_AUTH_TOKEN` fallback mechanism from the chat API routes (`clear`, `history`, and `persist`). All sensitive data operations now strictly require a properly verified Firebase ID token, preventing unauthorized access and bypassing of authentication validation.
