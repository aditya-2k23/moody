  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[400px] mx-auto flex flex-col gap-2">
        ...
        <button className="flex items-center justify-center gap-2 text-indigo-600 transition-colors px-4 py-2 rounded-full w-full max-w-[400px] mx-auto border-2 border-indigo-600 fugaz hover:opacity-70 duration-150" onClick={handleGoogleSignIn} disabled={authenticating}>
      ...
      } <Button onClick={() => setIsRegister(!isRegister)} className="text-indigo-600 hover:opacity-80" text={isRegister ? "Sign In" : "Sign Up"} normal={false} /></p>
    </div>
  )
}
