...
function initAdmin() {
  ...
  try {
    ...
    // Show truncated/sanitized version of the value for debugging
    const truncatedValue = serviceAccountKey.length > 100
      ? serviceAccountKey.substring(0, 100) + "..."
      : serviceAccountKey;
    const errorMsg = `[Firebase Admin] Failed to parse FIREBASE_SERVICE_ACCOUNT_KEY as JSON. ` +
      `Error: ${parseError.message}. ` +
      `Value starts with: "${truncatedValue.substring(0, 50)}..."`;
    console.error(errorMsg);
    throw new Error(errorMsg);
  }
